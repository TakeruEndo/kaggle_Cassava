...
def get_train_transforms(cfg):
    ...
    if da_version == 1:
        ...
        return Compose([
            ...
            RandomSunFlare(p=0.6, src_color=[160, 160, 160]),
            OneOf([
                ISONoise(p=1.0),
                GaussNoise(p=0.8),
                IAAAdditiveGaussianNoise(p=0.8),
            ...
            Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], max_pixel_value=255.0, p=1.0),
            OneOf([
                Cutout(num_holes=15, max_h_size=8, max_w_size=8, fill_value=0, always_apply=False, p=0.8),
                CoarseDropout(p=0.8),
            ], p=1.0),
            ToTensorV2(p=1.0),
        ], p=1.)
